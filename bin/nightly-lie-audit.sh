...
while IFS= read -r card; do
  TODAYS_CARDS+=("$card")
# EXCLUDE _runtime/* — those are per-machine session chunks (HEAD.md,
# chunk-NNN.md generated by walk-session-jsonl-nightly.sh on EACH machine
# locally). brain-sync explicitly does NOT propagate them (design intent).
# Including them in this check produces 63+ false-positive lies per run.
done < <(find "${HOME}/.claude/projects" -name "*.md" -path "*/memory/*" -newermt "${DATE} 00:00" 2>/dev/null \
          | grep -v '/_runtime/' \
          | head -50)
