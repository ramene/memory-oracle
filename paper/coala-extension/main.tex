...
on top of one. Crucially, it is also not a particular \emph{substrate}: the
property can be realised on top of any append-capable store---JSONL sidecars
on the file system, relational rows in PostgreSQL or Oracle 23ai, vector
chunks in MongoDB or pgvector, or graph nodes in a property
graph~\cite{oracleOAMP2026, mongoMemoriz2026}. The substrate-shape constraint
EBR imposes is on the \emph{retrieval contract} (Theorem~\ref{thm:precedence}),
not on the storage choice. This composability is what lets EBR be added as a
governance layer to existing production agent-memory products without
replacing them.
...
\subsection{Append-with-extraction versus append-with-supersession}
\label{sec:append-vs-supersession}

\noindent
The append-only property at the storage layer is necessary but not
sufficient for amendment-grade memory governance. Two distinct
architectures both append-only at the storage layer can have
fundamentally different retrieval semantics, with materially different
implications for HIPAA-grade workflows, financial audit, and clinical
correction.

The first architecture is \emph{append-with-extraction}: the substrate
appends raw messages, and an LLM extractor reads new messages and
decides what to write to the memory recall surface. The extractor's
policy is non-deterministic; the same input can produce three different
outcomes:

\begin{enumerate}
  \item A \emph{layered} write that preserves both prior records
        alongside the new one (memory recall returns multiple records;
        no structural rule says the new one dominates);
  \item A \emph{summarize-away} write that consolidates conflicting
        records into a derived synthesis and removes the originals from
        memory recall (though they remain in the underlying thread);
  \item An \emph{override} write that retains the new record and
        silently drops the prior.
\end{enumerate}

\noindent
This is the design of Oracle's Agent Memory Platform when
\texttt{extract\_memories=True}~\cite{oracleOAMP2026}, the default
configuration in the benchmark notebook~\cite{oracleAgentMemoryBenchmarks2026}.

The second architecture is \emph{append-with-supersession}: the substrate
appends amendment records that carry explicit \texttt{supersedes}
pointers to the canonical records they amend. At retrieval, the substrate
deterministically returns the head of the supersession
chain---the amendment---while preserving the canonical record for
historical retrieval via a separate query surface. This is EBR's design.

The two architectures converge on append-only at the \emph{storage}
layer but diverge at the \emph{retrieval} layer. Under
append-with-extraction, the dominant record at retrieval is determined
by extractor policy plus cosine ranking; under append-with-supersession,
the dominant record is determined by an explicit structural pointer.
For unregulated chat agents, the difference may be inconsequential; for
workflows where the retrieved value will be acted upon---clinical
treatment decisions, financial position adjustments, regulatory
amendments---only the structural guarantee suffices.

The distinction is empirically observable. The companion benchmark
notebook \texttt{oracle-style-comparison.ipynb} runs the same 80-turn
dialogue scenario through both architectures with a synthetic amendment
at turn 47. Among the three OAMP extractor policies, only
\emph{override} satisfies A1 (amendment dominates), only \emph{layer}
satisfies A2 (original preserved), and \emph{none} satisfies either A3
(structural chain) or A4 (typed provenance). EBR satisfies all four by
construction (Theorem~\ref{thm:precedence}). The full results matrix
appears in \S5.4 of the companion clinical-AI paper alongside the
HIPAA-grade discussion.

The implication for the CoALA framework is that EBR is not a substitute
for the four canonical memory types (working, semantic, procedural,
episodic) but a discipline that can be applied to the episodic-memory
layer of any of them. The append-with-supersession pattern is portable
to JSONL substrates, relational substrates, vector substrates, and graph
substrates with no change in semantics. The substrate-shape constraint
is on the retrieval contract, not on the storage choice.
...
\paragraph{Cryptographic signing --- substantially addressed via Verum.}
\label{para:verum}
The reference EBR substrate stores amendments as append-only JSONL on the
filesystem, with no cryptographic enforcement of authorship or immutability:
a malicious or accidental edit to an amendment line can change what the
substrate retrieves without leaving evidence beyond the file-system mtime.
We previously framed this as a near-term need for a Merkle-log or git-crypt
signing chain~\cite{shamir1979,gitcrypt2021}.

This open problem is substantially addressed by \emph{Verum}~\cite{verum2026},
a companion substrate forked from AGWA's git-crypt~\cite{gitcrypt2021} and
extended for the EBR pairing. Verum composes with the EBR file-layout
convention without modifying it: the canonical-file + amendment-sidecar
shape is preserved, but each amendment record gains cryptographic
authorship, the amendment chain becomes hash-linked, and the chain head
can be anchored on a public ledger. Table~\ref{tab:ebr-verum} maps each
open problem we previously named to the corresponding Verum mechanism.

\begin{table}[h]
\centering
\caption{EBR open problems mapped to Verum mechanisms.}
\label{tab:ebr-verum}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{p{0.27\textwidth}p{0.30\textwidth}p{0.33\textwidth}}
\toprule
\textbf{EBR open problem} & \textbf{Verum mechanism} & \textbf{Operator-visible property} \\
\midrule
Amendment authenticity (``who authored this correction?'') &
SSH/age signature on each amendment record; \texttt{add-age-recipient -{}-ssh} recipients are the signing identities &
The \texttt{amended\_by} field is cryptographically verifiable rather than a self-attested string. \\
\midrule
Amendment immutability (``did anyone backdate or tamper?'') &
Hash-chained access log + on-chain anchor (\texttt{audit-log}, \texttt{verify-audit}, \texttt{anchor-audit}) &
Filesystem-level tampering breaks the chain; the on-chain anchor proves the chain head's existence at time $T$. \\
\midrule
Patient-/operator-owned keys (this paper's title) &
Wallet-derived identity (\texttt{add-wallet-recipient}) + Shamir $M$-of-$N$ split (\texttt{split-key}, \texttt{unlock -{}-shares}) &
The operator owns the key outright; recovery requires $M$ designated share-holders to cooperate, with no third party able to reconstruct the key. \\
\midrule
Multi-author amendment provenance &
Multi-party {ACL} with SSH-signed proposals + role-based policy gates &
An amendment can require, e.g., attending + senior + compliance signatures before it commits; the signing chain is visible at retrieval time. \\
\midrule
Cross-substrate amendment portability &
Portable signed artifacts (\texttt{export ID FILE} / \texttt{import FILE}) + on-chain hash anchor &
An amendment's authenticity travels with the file; a receiving substrate verifies the signature and anchor independently. \\
\midrule
Pay-to-amend / EaaS-EBR &
{x402}~\cite{x402spec} HTTP licensing server (\texttt{git-crypt-license serve}) &
Operators running EBR-as-a-service can gate amendment-write operations behind issued per-user licenses, automated via the {x402} protocol. \\
\bottomrule
\end{tabular}
\end{table}

Five of the six open problems we previously named are now implemented in
Verum, leaving one open: \emph{multi-author precedence under partial
disagreement} --- when two co-equal operators write contradictory
amendments, the substrate currently lets the later one win by timestamp,
which is the wrong semantics for clinical consensus medicine and for any
setting where disagreement itself is the load-bearing signal. We treat
this as future work. The Verum substrate is licensed under GPLv3 and is
usable independently of the EBR file-layout convention for operators who
want the cryptographic guarantees without the substrate-level memory
semantics.

\paragraph{Context-card abstraction (\texttt{accretion.get\_citation\_card}).}
A useful pairing API---surfaced by the OAMP comparison in
\S\ref{sec:append-vs-supersession}---is a substrate-native
\texttt{get\_citation\_card(subject)} primitive that returns the head
amendment plus its supersession chain in a single bundled response. Oracle
exposes the analogous \texttt{thread.get\_context\_card()} for cosine-ranked
extracted memories; EBR's citation-card is its audit-grade counterpart,
returning the head plus the structurally-ordered chain back to the original
assertion. The two API shapes have different selection rules
(cosine-and-recency vs structural supersession) and different determinism
guarantees (probabilistic vs deterministic), but share an envelope so an
agent loop can route by workflow class. The detailed contrast appears in the
companion clinical-AI paper~\S7.4. Implementation is straightforward atop the
existing precedence-invariant query path; cross-language parity (Node and Go
adapters in the accretive-substrate reference repository) is in progress.
...
\noindent
\textbf{Substrate-layer agent-memory products.} A separate line of recent work ships
production agent-memory substrates as opinionated products. Oracle's Agent Memory
Platform (OAMP)~\cite{oracleOAMP2026} exposes a stack of \texttt{add\_memory},
\texttt{delete\_memory}, and \texttt{extract\_memories} primitives over a converged
Oracle 23ai vector + JSON + relational engine. The product page and developer guide
adopt the four CoALA memory types (working, semantic, procedural, episodic) as the
organising vocabulary without citing CoALA itself, which we note as a small datapoint
on the framework's reach into commercial deployment. The
amendment-grade assertion suite (\S\ref{sec:append-vs-supersession}) reveals that OAMP's
correction modes do not enforce the amendment-grade semantic by construction; this is
not a critique of OAMP as a substrate, but a clarification of where the substrate ends
and the governance layer begins. OAMP and EBR compose.

\noindent
\textbf{Anthropic Memory and Dreaming.} Anthropic's Memory and Dreaming
mode~\cite{anthropicMemoryDreaming2026} ships a session-consolidation architecture in
which an offline pass rewrites the agent's memory store via an LLM consolidator. The
public source release includes a consolidation prompt template
(\texttt{src/services/autoDream/consolidationPrompt.ts}) that instructs the model
verbatim: \emph{``Delete contradicted facts---fix it at the source.''} This is the
mutation semantic EBR is designed to prevent. We treat the public emergence of this
pattern as empirical evidence that the append-never-mutate constraint is not obvious
from first principles---major industry deployments are converging on
contradiction-as-deletion---and as direct motivation for the governance discipline EBR
codifies. Anthropic's architecture is otherwise sound; the operational concern is the
specific instruction the consolidator carries, not the consolidation step itself.

\noindent
\textbf{MongoDB Memoriz.} MongoDB's Memoriz library~\cite{mongoMemoriz2026} provides a
similar consolidate-and-replace pattern on top of Atlas Vector Search. The same
substrate-layer-vs-governance-layer framing applies: Memoriz answers the storage and
retrieval questions; EBR's precedence invariant and typed provenance are layerable on
top of it. The composition is unevaluated here but follows the same shape as the OAMP
composition.
