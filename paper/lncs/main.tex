...
real-money canary on Binance Futures''), and the supersession block
appears 30 lines before the canonical body in the merged retrieval. The
reproducibility script is
\texttt{docs/examples/trading-supersession-proof.sh}; the empirical notebook
...
\begin{enumerate}
  ...
  \item \textbf{The rules evolve weekly.} The operator's six May-10
        supersessions enumerated above include three rules that changed
        in a single day; their justifications cite prior supersession
        events (May 6--7 P\&L losses lowered a source's trust weight,
        which then triggered a threshold supersession --- a
        meta-supersession). An agent retrieving last week's rule on
...
memory-oracle (supersession-merged) & $\sim$1.0 & precedence invariant guarantees order \\
\bottomrule
\end{tabular}
\end{table}
...
operator caught the divergence in real time. A supersession sidecar
