...
\node[super, below=of curated] (super) {\textbf{Amendment records}\\\scriptsize \texttt{*.md.amendments.jsonl}\\\scriptsize (append-only)};
\node[digest, below=of super] (digest) {\textbf{Tier 2 — Journal digests}\\\scriptsize transcript-distilled summaries};

% --- Dashed boundary grouping the curated + amendment ---
...
\node[legend, below=0.4cm of digest.south, anchor=north, align=left, text width=12cm] {%
  \textbf{Precedence invariant:} for any retrieved file with $k \geq 1$ amendment records,
...
tiers (curated files, amendment records, journal digests) and merges
amendment records ahead of canonical bodies at read time. A filesystem
